...
export default function GameList() {
    ...
    return (
        <>
            ...
            {/* 배너는 스크롤되는 컨테이너 내부에 위치 */}
            <div className="GameList-container">
                <div className="banner-container">
                    <img src={bannerImg} alt="배너" className="banner-img" />
                </div>
                                    ...
                                    return (
                                        <div className="matchDetailRow">
                                            <table className="detail-table">
                                                <thead>
                                                <tr>
                                                    <th>챔프</th><th>닉네임</th><th>스펠</th><th>룬</th><th>K/D/A</th>
                                                    <th>라인</th><th>아이템</th><th>CS</th><th>가한피해량</th>
                                                    <th>받은피해</th><th>힐량</th><th>타워피해</th><th>시야점수</th>
                                                </tr>
                                                </thead>
                                                <tbody>
                                                {loseTeam.map(p => (
                                                    <tr key={p.puuid} className="red-row">
                                                        <td><img src={`https://ddragon.leagueoflegends.com/cdn/15.14.1/img/champion/${p.championName}.png`} width="24" height="24" alt={p.championName} /></td>
                                                        <td>{p.riotIdGameName}#{p.riotIdTagline}</td>
                                                        <td>
                                                            {[p.summoner1Id, p.summoner2Id].map((k, i) =>
                                                                keyToId[k] ? (
                                                                    <img key={i} src={`https://ddragon.leagueoflegends.com/cdn/15.14.1/img/spell/${keyToId[k]}.png`} width="20" height="20" alt="spell" />
                                                                ) : null
                                                            )}
                                                        </td>
                                                        <td>
                                                            {[p.primaryStyle1, p.primaryStyle2, p.primaryStyle3, p.subStyle1, p.subStyle2].map((u, i) => (
                                                                <img key={i} src={`https://ddragon.leagueoflegends.com/cdn/img/${u}`} width="20" height="20" alt="rune" />
                                                            ))}
                                                        </td>
                                                        ...
                                                        <td>
                                                            {[0,1,2,3,4,5,6].map(i => {
                                                                ...
                                                                const src = c && c !== '0'
                                                                    ...
                                                                    : `https://ddragon.leagueoflegends.com/cdn/15.14.1/img/profileicon/588.png`;
                                                                return <img key={i} src={src} width="20" height="20" alt={c && c !== '0' ? `item-${c}` : 'empty-slot'} />;
                                                            })}
                                                        </td>
                                                ...
                                                <tr className="label-row">
                                                    <td colSpan={13}>
                                                        ...
                                                        {JSON.parse(loseTeam[0].bans).map((ban, i) => (
                                                            <img key={i} className="ban-img" src={`https://ddragon.leagueoflegends.com/cdn/15.14.1/img/champion/${ban.championId}.png`} alt="ban" />
                                                        ))}
                                                        {statsData.map(([label, key]) => (
                                                            <span key={key} className="stat-text">{label}: {loseTeam[0][key]}</span>
                                                        ))}
                                                    </td>
                                                </tr>
                                                ...
                                                <tr className="label-row">
                                                    <td colSpan={13}>
                                                        ...
                                                        {statsData.map(([label, key]) => (
                                                            <span key={key} className="stat-text">{label}: {winTeam[0][key]}</span>
                                                        ))}
                                                    </td>
                                                </tr>
